...
Excluded: node_modules, build output, package-lock.json, and binary image/icon files.
...
function App() {
  ...
  const fetchAnalytics = async () => {
    try {
      const roomRes = await axios.get("http://localhost:5002/analytics/room");
      const deviceRes = await axios.get("http://localhost:5002/analytics/device");

      setRoomAnalytics(roomRes.data);
      setDeviceAnalytics(deviceRes.data);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const devicesRes = await axios.get("http://localhost:5001/devices");
  ...
  useEffect(() => {
    ...
    socket.on("deviceUpdate", fetchData);
  ...
  const toggleDevice = async (id) => {
    setDevices(prev =>
      prev.map(d => (d._id === id ? { ...d, status: !d.status } : d))
    );

    try {
      await axios.post(`http://localhost:5001/devices/toggle/${id}`);
    } catch {
      fetchData();
    }
  };
  ...
  // âœ… SAFE DATA (NO STACKING)
  const roomChartData = useMemo(() => ({
    labels: roomAnalytics.map(r => r._id),
    datasets: [
      {
        label: "Energy",
        data: roomAnalytics.map(r => r.total || 0),
  ...
  const deviceChartData = useMemo(() => ({
    labels: deviceAnalytics.map(d => d._id),
    datasets: [
      {
        label: "Devices",
        data: deviceAnalytics.map(d => d.total || 0),
  ...
  return (
    <div className="app">
      ...
      <div className="kpi">
        Total Energy: {energies.reduce((a, b) => a + b, 0).toFixed(2)} Wh
      </div>

      <div className="grid">
        {/* âœ… FIX: FORCE REMOUNT */}
        <div className="card">
          <h3>Energy by Room</h3>
          <Bar
            key={JSON.stringify(roomChartData)}
            data={roomChartData}
        ...
        <div className="card">
          <h3>Device Split</h3>
          <Pie
            key={JSON.stringify(deviceChartData)}
